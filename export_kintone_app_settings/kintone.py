    ...
    def export(self, app_ids):
        try:
            ...
            for app_id in app_ids:
                ...
                file_contents["一般設定"] = json.loads(resp.content.decode("utf-8"))
                # フィールドの一覧
                ...
                if resp.status_code != 200:
                    raise Exception('[Error] app_id: {}, status_code: {}, content: {}'
                                    .format(app_id, resp.status_code, resp.content.decode("utf-8")))
                file_contents["フィールドの一覧"] = json.loads(resp.content.decode("utf-8"))
                # フォームのレイアウト
                ...
                if resp.status_code != 200:
                    raise Exception('[Error] app_id: {}, status_code: {}, content: {}'
                                    .format(app_id, resp.status_code, resp.content.decode("utf-8")))
                file_contents["フォームのレイアウト"] = json.loads(resp.content.decode("utf-8"))
                # 一覧の設定
                ...
                if resp.status_code != 200:
                    raise Exception('[Error] app_id: {}, status_code: {}, content: {}'
                                    .format(app_id, resp.status_code, resp.content.decode("utf-8")))
                file_contents["一覧の設定"] = json.loads(resp.content.decode("utf-8"))
                # アプリのアクセス権
                ...
                if resp.status_code != 200:
                    raise Exception('[Error] app_id: {}, status_code: {}, content: {}'
                                    .format(app_id, resp.status_code, resp.content.decode("utf-8")))
                file_contents["アプリのアクセス権"] = json.loads(resp.content.decode("utf-8"))
                # レコードのアクセス権
                ...
                if resp.status_code != 200:
                    raise Exception('[Error] app_id: {}, status_code: {}, content: {}'
                                    .format(app_id, resp.status_code, resp.content.decode("utf-8")))
                file_contents["レコードのアクセス権"] = json.loads(resp.content.decode("utf-8"))
                # フィールドのアクセス権
                ...
                if resp.status_code != 200:
                    raise Exception('[Error] app_id: {}, status_code: {}, content: {}'
                                    .format(app_id, resp.status_code, resp.content.decode("utf-8")))
                file_contents["フィールドのアクセス権"] = json.loads(resp.content.decode("utf-8"))
                ...
                if resp.status_code != 200:
                    raise Exception('[Error] app_id: {}, status_code: {}, content: {}'
                                    .format(app_id, resp.status_code, resp.content.decode("utf-8")))
                file_contents["JavaScript/CSSカスタマイズ設定"] = json.loads(resp.content.decode("utf-8"))
                # プロセス管理の設定
                ...
                if resp.status_code != 200:
                    raise Exception('[Error] app_id: {}, status_code: {}, content: {}'
                                    .format(app_id, resp.status_code, resp.content.decode("utf-8")))
                file_contents["プロセス管理の設定"] = json.loads(resp.content.decode("utf-8"))
